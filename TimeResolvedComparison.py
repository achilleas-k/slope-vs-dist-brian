...
# neuron
neuron = NeuronGroup(len(S_in), "dV/dt = -V/(10*ms) : volt",
...
for idx, sync in enumerate(S_in):
    # inputs
    syncInp, randInp = genInputGroups(N_in, f_in, synx, 0*ms, duration, dt)
    syncGens.append(syncInp)
    randGens.append(randInp)
    netw.add(syncInp, randInp)
    # connections
    syncCon = Connection(syncInp, neuron, "V", weight=0.3*mV)
    randCon = Connection(randInp, neuron, "V", weight=0.3*mV)
...
for syncmon, randmon in zip(syncMons, randMons):
    ...
    inputSpikeTrainCollection.append(input_spiketrains)

BREAK HERE
CONTINUE WITH MULTI NEURON SIMULATION - ONE PER S_IN
# calculate npss
print("Calculating slope measure ...")
mslope, slopes = norm_firing_slope(vmon[0], outmon[0], 15*mV, 2*ms, dt)
_, otherslopes = npss(vmon[0], outmon[0], 15*mV, 2*ms, dt)

# calculate mean pairwise V-P distance for each interval
print("Calculating VP distance ...")
vp_dists = interval_VP(input_spiketrains, outmon[0], dcost)

# calculate multivariate Kreuz spike distance
print("Calculating Kreuz distance ...")
