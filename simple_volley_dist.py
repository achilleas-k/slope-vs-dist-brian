...
modm = modulus.interval(inputspikes, outspikes, 0, float(dura))

print("\tNPSS ...")
# calculate NPSS
norm_slopes = []
norm_slopes.append(npss(trace_mon[0], outspikes,
                        0*mV, Vth, tau, 2*ms))
...
for height, spiketrain in enumerate(input_mon.spiketimes.values()):
    plot(spiketrain/ms, zeros_like(spiketrain)+height, '.', c='gray')
ax2 = twinx()
ax2.plot(array(ptimes)/ms, sigmas, color='black', linestyle='--', linewidth=1.0)
subplot2grid((5,1), (4, 0))
for out in outspikes:
...
interval_midpoints = outspikes*1000
subplot(511)
#title("Packet widths")
...
subplot(513)
#title("Kreuz")
plot(interval_midpoints[1:], kreuz_d, 'k')
...
plot(interval_midpoints[1:], vp_d, 'k')
...
subplot(515)
#title("Modulus")
plot(interval_midpoints[1:], modm, 'k')
